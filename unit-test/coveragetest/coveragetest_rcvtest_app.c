...
/*
** File: coveragetest_rcvtest_app.c
**
** Purpose:
** Coverage Unit Test cases for the Rcvtest application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the Rcvtest application.
**
...
#include "rcvtest_app_coveragetest_common.h"
#include "ut_rcvtest_app.h"
...
void Test_RCVTEST_APP_Main(void)
{
    ...
    /*
     * RCVTEST_APP_Main does not return a value,
     ...
     * The first is for RCVTEST_APP_Init().  As this is in the same
     ...
     * the RCVTEST_APP_Init() failure path was taken.
     */
    RCVTEST_APP_Main();
     ...
     */
    UtAssert_True(RCVTEST_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "RCVTEST_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)RCVTEST_APP_Data.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, RCVTEST_APP_PIPE_ERR_EID, "RCVTEST APP: SB Pipe Read Error, App Will Exit");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "RCVTEST_APP_PIPE_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_RCVTEST_APP_Init(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(RCVTEST_APP_Init(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(RCVTEST_APP_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "CFE_ES_WriteToSysLog() called");

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UT_TEST_FUNCTION_RC(RCVTEST_APP_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UT_TEST_FUNCTION_RC(RCVTEST_APP_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 5, "CFE_ES_WriteToSysLog() called");
}

void Test_RCVTEST_APP_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void RCVTEST_APP_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        RCVTEST_APP_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, RCVTEST_APP_INVALID_MSGID_ERR_EID, "SAMPLE: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = RCVTEST_APP_CMD_MID;
    FcnCode   = RCVTEST_APP_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    RCVTEST_APP_ProcessCommandPacket(&TestMsg.SBBuf);

    TestMsgId = RCVTEST_APP_SEND_HK_MID;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    RCVTEST_APP_ProcessCommandPacket(&TestMsg.SBBuf);
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "RCVTEST_APP_COMMAND_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_RCVTEST_APP_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void RCVTEST_APP_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        RCVTEST_APP_NoopCmd_t          Noop;
        RCVTEST_APP_ResetCountersCmd_t Reset;
        RCVTEST_APP_ProcessCmd_t       Process;
    } TestMsg;
    UT_CheckEvent_t EventTest;
    ...
    UT_CheckEvent_Setup(&EventTest, RCVTEST_APP_COMMANDNOP_INF_EID, NULL);

    RCVTEST_APP_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_True(EventTest.MatchCount == 1, "SAMPLE_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = RCVTEST_APP_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CheckEvent_Setup(&EventTest, RCVTEST_APP_COMMANDRST_INF_EID, NULL);
    ...
    /* note this will end up calling RCVTEST_APP_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = RCVTEST_APP_PROCESS_CC;
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CheckEvent_Setup(&EventTest, RCVTEST_APP_COMMAND_ERR_EID, "Invalid ground command code: CC = %d");
    RCVTEST_APP_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_RCVTEST_APP_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void RCVTEST_APP_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(RCVTEST_APP_SEND_HK_MID);

    /* Set message id to return so RCVTEST_APP_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    RCVTEST_APP_ProcessCommandPacket((CFE_SB_Buffer_t *)NULL);

    /* Confirm message sent*/
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TransmitMsg)) == 1, "CFE_SB_TransmitMsg() called once");
    UtAssert_True(MsgSend == &RCVTEST_APP_Data.HkTlm.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &RCVTEST_APP_Data.HkTlm.TlmHeader.Msg,
...
void Test_RCVTEST_APP_NoopCmd(void)
{
    /*
     * Test Case For:
     * void RCVTEST_APP_NoopCmd( const RCVTEST_APP_Noop_t *Msg )
     */
    RCVTEST_APP_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;
    ...
    UT_TEST_FUNCTION_RC(RCVTEST_APP_Noop(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "RCVTEST_APP_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_RCVTEST_APP_ResetCounters(void)
{
    /*
     * Test Case For:
     * void RCVTEST_APP_ResetCounters( const RCVTEST_APP_ResetCounters_t *Msg )
     */
    RCVTEST_APP_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CheckEvent_Setup(&EventTest, RCVTEST_APP_COMMANDRST_INF_EID, "SAMPLE: RESET command");

    UT_TEST_FUNCTION_RC(RCVTEST_APP_ResetCounters(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "RCVTEST_APP_COMMANDRST_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_RCVTEST_APP_ProcessCC(void)
{
    /*
     * Test Case For:
     * void  RCVTEST_APP_ProcessCC( const RCVTEST_APP_Process_t *Msg )
     */
    RCVTEST_APP_ProcessCmd_t TestMsg;
    RCVTEST_APP_Table_t      TestTblData;
    void *                  TblPtr = &TestTblData;

    memset(&TestTblData, 0, sizeof(TestTblData));
    memset(&TestMsg, 0, sizeof(TestMsg));

    /* Provide some table data for the RCVTEST_APP_Process() function to use */
    TestTblData.Int1 = 40;
    TestTblData.Int2 = 50;
    UT_SetDataBuffer(UT_KEY(CFE_TBL_GetAddress), &TblPtr, sizeof(TblPtr), false);
    UT_TEST_FUNCTION_RC(RCVTEST_APP_Process(&TestMsg), CFE_SUCCESS);
     ...
     */
    UT_SetDefaultReturnValue(UT_KEY(CFE_TBL_GetAddress), CFE_TBL_ERR_UNREGISTERED);
    UT_TEST_FUNCTION_RC(RCVTEST_APP_Process(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
}

void Test_RCVTEST_APP_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool RCVTEST_APP_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
     ...
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, RCVTEST_APP_LEN_ERR_EID,
                        "Invalid Msg length: ID = 0x%X,  CC = %u, Len = %u, Expected = %u");

    RCVTEST_APP_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "RCVTEST_APP_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    RCVTEST_APP_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "RCVTEST_APP_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_RCVTEST_APP_TblValidationFunc(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(RCVTEST_APP_TblValidationFunc(&TestTblData), CFE_SUCCESS);

    /* error case should return RCVTEST_APP_TABLE_OUT_OF_RANGE_ERR_CODE */
    TestTblData.Int1 = 1 + RCVTEST_APP_TBL_ELEMENT_1_MAX;
    UT_TEST_FUNCTION_RC(RCVTEST_APP_TblValidationFunc(&TestTblData), RCVTEST_APP_TABLE_OUT_OF_RANGE_ERR_CODE);
}

void Test_RCVTEST_APP_GetCrc(void)
{
    /*
     * Test Case For:
...
{
    ADD_TEST(RCVTEST_APP_Main);
    ADD_TEST(RCVTEST_APP_Init);
    ADD_TEST(RCVTEST_APP_ProcessCommandPacket);
    ADD_TEST(RCVTEST_APP_ProcessGroundCommand);
    ADD_TEST(RCVTEST_APP_ReportHousekeeping);
    ADD_TEST(RCVTEST_APP_NoopCmd);
    ADD_TEST(RCVTEST_APP_ResetCounters);
    ADD_TEST(RCVTEST_APP_ProcessCC);
    ADD_TEST(RCVTEST_APP_VerifyCmdLength);
    ADD_TEST(RCVTEST_APP_TblValidationFunc);
    ADD_TEST(RCVTEST_APP_GetCrc);
}
